...
def vehicle_info():
    reg_no = request.args.get("vehicle", "").strip().upper()

    if not reg_no:
        return jsonify({
            "status": False,
            "error": "Missing vehicle parameter",
            "example": "/api/info?vehicle=RJ14TD6311"
        }), 400
    ...
    if not search_data:
        return jsonify({"status": False, "message": "No data found or invalid vehicle number"}), 404
